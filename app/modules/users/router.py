...
async def check_logged_in_status(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return create_response(result=result, pydantic_model=UserLoggedInStatusResponse, message="User authenticate successfully")
...
async def get_users(page:int=1, limit:int=10,email:str= None, db: AsyncSession = Depends(get_db)):
    ...
    return create_response(result=result["data"], pydantic_model=UsersListResponse, message="Users have retrieved successfully", meta_data=result["meta_data"] )